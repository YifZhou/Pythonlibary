...
def interpBadPixels(image, mask):
    ...
    """
    # fix the bad pixel by interpolating on the row, for which the gradient is smaller
    fixedIm = image.copy()
    dim = image.shape[0]
    for i in range(dim):
        im_i = image[i, :]
        mask_i = mask[i, :]
        fixedrow = im_i.copy()
        goodx = np.where(mask_i == 0)[0]
        badx = np.where(mask > 0)[0]
        interpf = interp1d(goodx, im_i[goodx], kind='linear', fill_value='extrapolate')
        # interpv = griddata((goodx), im_i[goodx], (badx), method='linear')
        interpv = interpf(badx)
        fixedrow[badx] = interpv
        fixedIm[i, :] = fixedrow
...
def padimg(fname, zero=True, correct=True, skyname=None, flatname=None):
    ...
    if flatname is not None:
        ...
        with fits.open(flatname) as flatf:
            for i in xrange(4):  # flat field is 3 order polynomial
